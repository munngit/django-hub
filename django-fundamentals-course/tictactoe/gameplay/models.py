...
class Game(models.Model):
    ...
    #
    #     board = [
    #         [None for x in range(BOARD_SIZE)]
    ...
    def board(self):
        """Returns a 2D list with 'X', 'O', or None (empty cells)."""
        board = [["" for _ in range(BOARD_SIZE)] for _ in range(BOARD_SIZE)]
        for move in self.move_set.all():
            board[move.y][move.x] = "X" if move.by_first_player else "O"
        return board
    ...
    def new_move(self):
        ...
        if self.status not in 'FS':  # Ensure the game is still active
            raise ValueError("Cannot make move on a finished game")

        return Move(
            game=self,
            by_first_player=self.status == 'F'  # Set turn based on the game's status
        )
...
class Move(models.Model):
    ...
    by_first_player = models.BooleanField(null=True,editable=False)
